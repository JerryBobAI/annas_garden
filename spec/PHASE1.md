...
- [x] 新用户首次进入 → Onboarding 流程完整 ✅
- [x] 探索模式对话 → 精灵回复包含情绪和选项 ✅
- [x] 任务模式对话 → 精灵生成情境化题目 ✅
- [x] 创造模式对话 → 精灵引导创作 ✅
- [x] 长对话 → token 截断不丢失上下文 ✅（token-manager 测试覆盖）
- [x] 网络断开 → 显示友好错误 ✅（error.tsx 错误边界）
- [x] iPad Safari → 触摸和滚动正常 ✅
...
- [x] **F1**: 新用户第一次进入孩子端 → 看到 Onboarding 引导流程 ✅
- [x] **F2**: Onboarding 完成 → 自动进入第一次 AI 对话 ✅
- [x] **F3**: 探索模式 → 孩子输入问题 → 精灵流式回复 + 结构化输出 ✅
- [x] **F4**: 任务模式 → 精灵生成情境化题目 + 提供选项按钮 ✅
- [x] **F5**: 创造模式 → 精灵引导孩子创作 ✅
- [x] **F6**: 选项按钮 → 点击后作为用户消息发送 ✅
- [x] **F7**: 对话历史 → 刷新页面后对话还在 ✅
- [x] **F8**: 长对话 → 超过 token 限制时自动截断旧消息 ✅
- [x] **F9**: 精灵情绪 → 根据 AI 回复切换（happy/thinking/surprised/cheering） ✅
- [x] **F10**: 错误处理 → 网络断开/API 失败显示友好提示 + 重试按钮 ✅

### 技术验收

- [x] **T1**: 数据库迁移 002 成功执行，6 张新表 + RLS ✅
- [x] **T2**: `lib/ai/structured-output.ts` 单元测试通过（≥5 个用例） ✅
- [x] **T3**: `lib/ai/token-manager.ts` 单元测试通过（≥4 个用例） ✅
- [x] **T4**: `/api/ai/chat` 返回流式 SSE 响应 ✅
- [x] **T5**: 孩子端有独立 layout + ErrorBoundary + Loading ✅
- [x] **T6**: 底部导航从页面提取到 layout，所有子页面共享 ✅

### 体验验收

- [x] **U1**: iPad Safari 触摸无卡顿 ✅
- [x] **U2**: 对话首次响应 < 3 秒（首 token 到达） ✅
- [x] **U3**: 精灵情绪切换有动画过渡 ✅
- [x] **U4**: 对话气泡有出场动画 ✅
- [x] **U5**: 输入框在键盘弹起时不被遮挡 ✅
