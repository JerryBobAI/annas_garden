...
- [ ] **C1**: 故事创作完整流程（构思 → 共创 → 完成 → 阅读）
- [ ] **C2**: 数学探索完整流程（情境 → 做题 → 可视化 → 结果）
- [ ] **C3**: 英语冒险完整流程（场景 → 对话 → 单词 → 发音）
- [ ] **C4**: 创作自动保存到数据库
- [ ] **C5**: 创作列表按类型/学科筛选
- [ ] **C6**: 创作收藏功能
- [ ] **C7**: 创作完成触发花园植物开花
- [ ] **C8**: 数学答题结果记录到 knowledge_mastery

### 技术验收

- [ ] **T1**: 数据库迁移 003 成功执行
- [ ] **T2**: creations + creation_pages 的 CRUD + RLS 正常
- [ ] **T3**: 创造模式对话自动保存逻辑有测试
- [ ] **T4**: 三种创造 prompt 各有至少 3 轮测试对话
